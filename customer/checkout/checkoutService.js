...
async function createNewOrder(user_id, total, address) {
    try {
        // Generate a random order code: 8 characters alphanumeric, uppercase
        const order_code = Math.random().toString(36).substring(2, 10).toUpperCase();

        const result = await pool.query(
            `INSERT INTO orders (user_id, total, address, order_code, status, status_payment) 
             VALUES ($1, $2, $3, $4, 'Pending', 'Unpaid') RETURNING order_id, order_code`,
            [user_id, total, address, order_code]
        );

        const order_id = result.rows[0].order_id;
        // const returned_order_code = result.rows[0].order_code; // Should match the generated one
