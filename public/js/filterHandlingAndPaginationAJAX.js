...
function updateProductList(products) {
  ...
  products.forEach((product) => {
    ...
    if (product.is_discount_active && product.discount > 0) {
      ...
      const discountedPrice = parseInt(originalPrice * (1 - product.discount / 100));
      discountPriceHTML = `
    ...
    let productHTML = `
      <div class="bg-white shadow-md flex flex-col h-full rounded-lg">
        ...
        <a href="#" class="add-to-cart-btn block w-full py-3 mt-auto text-center text-white bg-[#9c5e25] hover:bg-[#b87434] transition rounded-b-lg" data-product-id="${product.product_id}" data-product-price="${product.price}">
          Thêm vào giỏ hàng
        </a>
      </div>`;
